...
<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents cart__table cart-table" cellspacing="0">
		<thead class="cart-table__head">
			<tr class="cart-table__row">
				<th class="product-thumbnail cart-table__column cart-table__column--image">&nbsp;<?php esc_html_e( 'Image', 'woocommerce' ); ?></th>
				<th class="product-name cart-table__column cart-table__column--product"><?php esc_html_e( 'Product', 'woocommerce' ); ?></th>
				<th class="product-price cart-tablsse__column cart-table__column--price"><?php esc_html_e( 'Price', 'woocommerce' ); ?></th>
				<th class="product-quantity cart-table__column cart-table__column--quantity"><?php esc_html_e( 'Quantity', 'woocommerce' ); ?></th>
				<th class="product-subtotal cart-table__column cart-table__column--total"><?php esc_html_e( 'Subtotal', 'woocommerce' ); ?></th>
		...
		<tbody class="cart-table__body">
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					?>
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?> cart-table__row">
						...
						if ( ! $product_permalink ) {
							...
							printf( '<div class="product-image"><a class="product-image__body" href="%s">%s</a></div>', esc_url( $product_permalink ), $thumbnail ); // PHPCS: XSS ok.
						}
						?>
						</td>
						...
						<td class="product-subtotal cart-table__column cart-table__column--total" data-title="<?php esc_attr_e( 'Subtotal', 'woocommerce' ); ?>">
							<?php
								echo apply_filters( 'woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal( $_product, $cart_item['quantity'] ), $cart_item, $cart_item_key ); // PHPCS: XSS ok.
							?>
						</td>

                        <td class="product-remove cart-table__column cart-table__column--remove">
                            <?php
    ...
    <div class="cart__actions">
        <?php if ( wc_coupons_enabled() ) { ?>
            <div class="coupon cart__coupon-form">
        ...
        <div class="cart__buttons">
            <button type="submit" class="button btn btn-primary cart__update-button" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'woocommerce' ); ?>"><?php esc_html_e( 'Update cart', 'woocommerce' ); ?></button>
...
<div class="row justify-content-end pt-5">
    <div class="col-12 col-md-7 col-lg-6 col-xl-5">
        <div class="cart-collaterals card">
	<?php
		/**
